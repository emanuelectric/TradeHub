...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    public List<SimboloModel> getEveryone(){
        List<SimboloModel> returnList = new ArrayList<>();

        String queryString = "SELECT * FROM " + SIMBOLO;
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(queryString, null);

        if(cursor.moveToFirst()){
            do{
                int simmboloID = cursor.getInt(0);
                String simboloNombre = cursor.getString(1);
                String nombre = cursor.getString(2);
                String descripcionSimbolo = cursor.getString(3);

                SimboloModel nuevoSimbolo = new SimboloModel(simmboloID,simboloNombre, nombre,descripcionSimbolo);
                returnList.add(nuevoSimbolo);
